"""View decorators for the vendors module."""
...
def tenant_admin_required(view_func):
    ...
    D-10 remediation: `@login_required` alone lets any authenticated tenant user
    delete/update vendor data. This decorator gates destructive operations behind
...
def emit_audit(request, action, instance, changes=''):
    """Emit a `core.AuditLog` row for a vendor-module action.

    D-09 remediation. Silently no-ops if `request.tenant` is unset (superuser
